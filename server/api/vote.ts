...
import { VoteTimes as VOTE_TIMES } from "../constants/voteTimes";
...
router.post("/", async (req, res) => {
    ...
    if (foundUser.vote.date !== null && VOTE_TIMES.TWELVE_HOURS - (Date.now() - foundUser.vote.date.getTime()) > 0)
        return res.status(403).json({ message: "You can't vote again yet!", error: "Forbidden." });
    ...
    WebSocket.emit("new-vote", foundUser, foundBot);
    WebSocket.emit("vote", foundUser.vote, foundUser.id);
